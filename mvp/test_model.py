...
sc = SparkSession.builder.appName("TextRead").getOrCreate()
...
#tk = Tokenizer(inputCol="_c2", outputCol="words")
...
sentenceData = sc.createDataFrame([(0.0, text),],['label','sentence'])
...
data = sc.read.format("parquet").load(parquetpath)
# tf = hashingTF.transform(words)
# tf.cache()
...
sim = data.select("features").rdd.map(lambda x: cos(x, sample))
