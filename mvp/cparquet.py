...
sc =  SparkSession.builder.appName("Count Parquet").getOrCreate()
...
#dd = sc.read.parquet(parquetpath).select(create_map('id','title','tags','creation_date').alias('map'))
dd = sc.read.parquet(parquetpath)
dd = dd.withColumn('towrite',redis_udf(struct([dd[x] for x in dd.columns])))
dd.show(20)
#dd = sc.read.parquet(parquetpath).rdd.map(store_redis).show()
#print(dd)
